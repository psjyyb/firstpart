module.exports = {
    ...
    mypageYesReviewCount:'SELECT COUNT(*) as ycnt FROM orders o JOIN order_detail d ON o.order_no = d.order_no JOIN product p ON p.product_no = d.product_no LEFT JOIN review r ON r.order_no = d.order_no AND r.product_no = d.product_no WHERE o.user_id = ? AND r.review_no IS NULL',
    mypageNoReviewCount:'SELECT COUNT(*) as ncnt FROM orders o JOIN order_detail d ON o.order_no = d.order_no JOIN product p ON p.product_no = d.product_no JOIN review r ON r.order_no = d.order_no AND r.product_no = d.product_no WHERE o.user_id = ?',
